...
```json
{
    "UserId": "AIDAW7UO15ZLC6DNUO15Z",
    "Account": "480134881234",
    "Arn": "arn:aws:iam::480134881234:user/terraform-beginner-bootcamp"
}
```

We'll need to generate AWS CLI credencials from IAM User in order to the user AWS CLI.
...
#### Bucket Name
While creating S3 bucket we found out that bucket name should be lower case and numbers.

#### Error while doing Terraform destroy.

When tring to destroy the S3 bucket I was getting the access denied error 403. So I deleted the bucket in the Aws console and done the **terraform refresh** to sync the changes with the state fila and the resources in the AWS console.
